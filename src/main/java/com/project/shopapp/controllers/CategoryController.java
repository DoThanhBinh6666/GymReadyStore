...
public class CategoryController {
    ...
    // Tìm kiếm các category chưa kích hoạt theo từ khóa
    @GetMapping("/inactive")
    public ResponseEntity<ResponseObject> searchInactiveCategories(
            @RequestParam(required = false, defaultValue = "") String keyword) {
        List<Category> inactiveCategories = categoryService.searchInactiveCategories(keyword);
        return ResponseEntity.ok(
                new ResponseObject("Inactive categories retrieved successfully", HttpStatus.OK, inactiveCategories));
    }

    // Cập nhật trạng thái category về active = true
    @PutMapping("/activate/{id}")
    public ResponseEntity<ResponseObject> activateCategory(@PathVariable long id) {
        boolean isActivated = categoryService.activateCategory(id);
        if (isActivated) {
            return ResponseEntity.ok(
                    new ResponseObject("Category activated successfully", HttpStatus.OK, null));
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ResponseObject("Category not found or already active", HttpStatus.NOT_FOUND, null));
        }
    }
}
