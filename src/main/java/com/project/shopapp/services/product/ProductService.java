...
public class ProductService implements IProductService{
    ...
    public void deleteProduct(long id) {
        ...
        optionalProduct.ifPresent(product -> {
            ...
            productRepository.save(product); // Save the updated product with active = false
        });
    }
    ...
    public void activateProduct(Long productId) throws Exception {
        ...
        // Kiểm tra trạng thái active của category
        if (product.getCategory() != null && product.getCategory().getActive()) {
            // Cập nhật trạng thái active của sản phẩm
            product.setActive(true);
            productRepository.save(product);
        } else {
            throw new Exception("Product's category is inactive");
        }
    }
    @Override
    public Page<ProductResponse> searchInactiveProducts(String keyword, PageRequest pageRequest) {
        // Sử dụng repository để tìm kiếm sản phẩm với active = false
        Page<Product> productsPage = productRepository.searchInactiveProducts(keyword, pageRequest);
        return productsPage.map(ProductResponse::fromProduct);
    }
    @Override
    @Transactional
    public Product updateInactiveProduct(long id, ProductDTO productDTO) throws Exception {
        // Lấy sản phẩm bất kể trạng thái active/inactive
        Product existingProduct = productRepository.findById(id)
                .orElseThrow(() -> new DataNotFoundException("Cannot find product with id: " + id));

        // Kiểm tra nếu sản phẩm đang active, thì không cho phép cập nhật
        if (existingProduct.isActive()) {
            throw new Exception("Cannot update an active product using this method");
        }

        // Tiếp tục cập nhật thông tin của sản phẩm
        Category existingCategory = categoryRepository
                .findById(productDTO.getCategoryId())
        ...
        // Lưu sản phẩm đã cập nhật và trả về
        return productRepository.save(existingProduct);
    }
