...
public class UserService implements IUserService{
    ...
    @Override
    public Page<User> findInactiveUsers(String keyword, Pageable pageable) throws Exception {
        return userRepository.findInactiveUsers(keyword, pageable);
    }

    @Override
    public void activateUser(Long userId) throws DataNotFoundException {
        int updatedRows = userRepository.activateUser(userId);
        if (updatedRows == 0) {
            throw new DataNotFoundException("User with ID " + userId + " not found or already active.");
        }
    }
}
